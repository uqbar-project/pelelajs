...
function extractClassMembers(line, properties, methods) {
  const propMatch = /^\s*(?:public\s+|private\s+|protected\s+)?([a-zA-Z_]\w*)\s*(?::|=)\s*/.exec(line);
  if (propMatch) {
    properties.add(propMatch[1]);
    return;
  }

  const getterMatch = /^\s*(?:public\s+|private\s+|protected\s+)?get\s+([a-zA-Z_]\w*)\s*\(/.exec(line);
  if (getterMatch) {
...
function findPropertyLine(lines, propertyName) {
  const rootPropRegex = new RegExp(`^\\s*(?:public\\s+|private\\s+|protected\\s+)?${propertyName}\\??\\s*[=:]`, "m");
