...
const errors = {
  ...
  viewmodel: {
    ...
    export: {
      missingExport:
        '[pelela] The class "{{viewModelName}}" must be exported in {{tsFilePath}}. Add the export keyword to the class.',
      wrongCase:
  ...
  routing: {
    ...
    multipleOutlets:
      '[pelela] Layout template must contain exactly one <outlet> element. Found {{count}}.',
    outletWithChildren:
